...
$qery = "SELECT   tblstudents.StudentName,tblstudents.RollId,tblstudents.RegDate,tblstudents.StudentId,tblstudents.Status,tblclasses.ExamName,tblclasses.Section from tblstudents join tblclasses on tblclasses.id=tblstudents.ExamId where tblstudents.RollId=? and tblstudents.ExamId=?";
$stmt21 = $mysqli->prepare($qery);
$stmt21->bind_param("ss",$rollid,$examid);
$stmt21->execute();
...
<p><b>Student Class:</b> <?php echo htmlentities($result->ExamName);?>(<?php echo htmlentities($result->Section);?>)
<?php }
...
$stmt->bind_param("ss",$rollid,$examid);
$stmt->execute();
                 $res=$stmt->get_result();
                 $cnt=1;
