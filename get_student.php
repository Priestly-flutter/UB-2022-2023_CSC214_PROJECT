...
if(!empty($_POST["examid"]))
{
 ...
 if(!is_numeric($cid)){
 	...
 	echo htmlentities("invalid Exam");exit;
 }
 else{
...
// Code for Subjects
if(!empty($_POST["examid1"]))
{
 $cid1=intval($_POST['examid1']);
 if(!is_numeric($eid1)){
 ...
 $status=0;	
 $stmt = $dbh->prepare("SELECT tblsubjects.SubjectName,tblsubjects.id FROM tblsubjectcombination join  tblsubjects on  tblsubjects.id=tblsubjectcombination.SubjectId WHERE tblsubjectcombination.ExamId=:eid and tblsubjectcombination.status!=:stts order by tblsubjects.SubjectName");
 $stmt->execute(array(':eid' => $eid1,':stts' => $status));
...
if(!empty($_POST["studexam"]))
{
 $id= $_POST['studexam'];
 $dta=explode("$",$id);
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT StudentId,ClassId FROM tblresult WHERE StudentId=:id1 and ExamId=:id ");
